...
// Lots o' modifications by Joe Grand (Kingpin) for the DEFCON 17 Badge
...
char StrCopyright[] = "\n\n\r\
DEFCON 17 Badge by Joe Grand (Kingpin)\n\r\
www.grandideastudio.com + www.kingpinempire.com\n\r\
-----------------------------------------------\n\r"; 
...
void main(void)
{
  ...
  /****************************************************************************/
    ...
    Cpu_Delay100US(5000); // power-up delay to make sure battery is in place and VCC has settled
    ...
    if(TMR1_SCTRL & TMR1_SCTRL_TCF_MASK) // is true every 1 second
    {
        ...
        if (--StartDelay == 0) 
        {
          ...
          bootExit();	
			...
			// turn on each R, G, and B LED individually to aid in manufacturing test
			// also lets the user know that we are in bootloader mode
   		switch (StartDelay % 3)
  		{		
  			case 0:	// red
  				Bits1_PutVal(0b110);
